...
client.on('message', message => {
  ...
  if (message.author.id !== "184221825633484800") return;
...
client.on('message', message => {
   ...
   let embed = new Discord.RichEmbed()
    let args = message.content.split(' ').slice(1).join(' ');
if(message.content.split(' ')[0] == prefix + 'sbc') {
    if (!args[1]) {
return;
}
...
let embed = new Discord.RichEmbed()
let args = message.content.split(' ').slice(1).join(' ');
if(message.content.split(' ')[0] == prefix + 'sbc1') {
...
client.on("message", message => {
            ...
            if (message.content.startsWith(prefix + "sbc2")) {
                         if (!message.member.hasPermission("ADMINISTRATOR"))  return;
